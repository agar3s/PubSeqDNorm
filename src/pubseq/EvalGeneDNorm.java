...
import pubseq.core.GeneLexiconLoader;
import pubseq.core.GeneNameAnalyzer;
import pubseq.core.GeneSynonymTrainer;
import pubseq.core.GeneSynonymTrainer.LookupResult;
...
/**
 * Gene implementation of {@link EvalDNorm}. Some fields and methods in this
 * class might need to be modified. However, it is suggested that later
 * implementation should only include modification of class {@link EvalDNorm}.
 * 
 * @author raharjaliu
 *
 */
public class EvalGeneDNorm {

	public GeneNameAnalyzer analyzer;
	public GeneLexicon lex;
	public GeneLexiconLoader loader; // FIXME
	public AbbreviationResolver abbrev;
	public HierarchicalConfiguration config;
	public Dataset dataset;
	public CRFTagger tagger;

	public EvalGeneDNorm(GeneNameAnalyzer analyzer, String lexiconFilename,
			String abbreviationFilename) {
		this.analyzer = analyzer;
		lex = new GeneLexicon(analyzer);
		loader = new GeneLexiconLoader();
		loader.loadLexicon(lex, lexiconFilename);
		abbrev = new AbbreviationResolver();
		abbrev.loadAbbreviations(abbreviationFilename);
	}

	public GeneLexicon getLexicon() {
		return lex;
	}
		...
		for (Sentence sentence : dataset.getSentences()) {
			...
			for (Mention mention : sentence.getMentions()) {
				...
				for (String conceptId : conceptIds) {
					...
					correctAnnotations.add(new AbstractAnnotation(documentId,
							conceptId));
				}
			}
		}
	...
	public List<Set<AbstractAnnotation>> getFoundAnnotations(Dataset dataset,
			banner.tagging.Tagger tagger, GeneSynonymTrainer syn,
			HierarchicalConfiguration config, int maxRank) throws IOException {
		...
		for (Sentence sentence : dataset.getSentences()) {
			Sentence sentence2 = BANNER.process(tagger, tokenizer,
			...
			for (Mention mention : sentence2.getMentions(MentionType.Found)) {
				...
				for (int i = 0; i < Math.min(results.size(), maxRank); i++) {
					...
					foundAnnotations.get(i).add(
							new AbstractAnnotation(documentId, conceptId));
				}
			}
		}
	...
	private void visualize(Lookup lookup, String lookupString) {
		...
		for (int i = 0; i < hits.length; i++) {
			...
			System.out.printf("\t%f\t%s\t%s", hits[i].score,
					doc.get("conceptId"), lex.visualizeVector(doc.get("name")));
			System.out.println();
		}
		System.out.println();
	}

	public void prepareBANNER(String configurationFile)
	...
	private void outputAnalysis(Dataset dataset, banner.tagging.Tagger tagger,
			GeneSynonymTrainer syn, HierarchicalConfiguration config,
			int maxRank, String outputFilename,
			Set<AbstractAnnotation> correctAnnotations,
			Set<AbstractAnnotation> foundAnnotations) throws IOException {
		Tokenizer tokenizer = BANNER.getTokenizer(config);
		PostProcessor postProcessor = BANNER.getPostProcessor(config);
		...
		Set<AbstractAnnotation> falseNegatives = new HashSet<AbstractAnnotation>(
				correctAnnotations);
		...
		for (Sentence sentence : sentences) {
			...
			if (!sentence.getDocumentId().equals(currentDocument)) {
				...
				for (AbstractAnnotation a : falseNegatives) {
					if (a.getDocumentId().equals(currentDocument)) {
						...
						Set<String> names = lex.getNamesForConcept(a
								.getConceptId());
						writer.write("\t" + names);
						writer.newLine();
					}
			...
			writer.write(sentence.getDocumentId() + "\t"
					+ sentence.getSentenceId() + "\t" + sentence.getText());
			writer.newLine();
			Sentence sentence2 = BANNER.process(tagger, tokenizer,
					postProcessor, sentence);
			String documentId = sentence2.getDocumentId();
			for (Mention mention : sentence2.getMentions(MentionType.Found)) {
				...
				for (int i = 0; i < Math.min(results.size(), maxRank); i++) {
					...
					String output = String.format("\t%s\t%d\t%f", marker, i,
							results.getValue(i));
					output += String.format("\t%s\t%s", results.getObject(i)
							.getConceptId(), lex.visualizeVector(results
							.getObject(i).getVector()));
	...
	private String calculateMicroAveragedFMeasure(
			Set<AbstractAnnotation> correctAnnotations,
			Set<AbstractAnnotation> foundAnnotations) {
		Set<AbstractAnnotation> tpSet = new HashSet<AbstractAnnotation>(
				correctAnnotations);
	...
	private String calculateMacroAveragedFMeasure(
			Set<AbstractAnnotation> correctAnnotations,
			Set<AbstractAnnotation> foundAnnotations) {
		...
		for (AbstractAnnotation a : correctAnnotations) {
			...
			Set<AbstractAnnotation> annotations = correctAnnotationsMap
					.get(documentId);
			if (annotations == null) {
		...
		for (AbstractAnnotation a : foundAnnotations) {
			String documentId = a.getDocumentId();
			Set<AbstractAnnotation> annotations = foundAnnotationsMap
					.get(documentId);
			if (annotations == null) {
				annotations = new HashSet<AbstractAnnotation>();
	...
	private String calculateRecallCeiling(
			Set<AbstractAnnotation> correctAnnotations,
			List<Set<AbstractAnnotation>> foundAnnotations, int maxRank) {
		...
		int fn = correctAnnotations.size() - tp;
		double r = ((double) tp) / (tp + fn);
		return "Ceiling @ " + maxRank + "\tMax\t" + foundAnnotations.size()
				+ "\tR\t" + r;
	}

	private void runSynonymSimilarity(String configurationFile,
			String outputFileName, SynonymMatrix matrix)
			throws ConfigurationException, IOException {
		int maxRank = 10;
		long start = System.currentTimeMillis();
		GeneSynonymTrainer syn = new GeneSynonymTrainer(lex, matrix, 1000);
		prepareBANNER(configurationFile);
		// HierarchicalConfiguration localConfig =
		// config.configurationAt(BANNER.class.getPackage().getName());
		Set<AbstractAnnotation> correctAnnotations = prepareCorrectAnnotations(dataset);
		List<Set<AbstractAnnotation>> foundAnnotations = getFoundAnnotations(
				dataset, tagger, syn, config, maxRank);
		outputAnalysis(dataset, tagger, syn, config, maxRank, outputFileName,
				correctAnnotations, foundAnnotations.get(0));
		System.out.println(calculateMacroAveragedFMeasure(correctAnnotations,
				foundAnnotations.get(0)));
		System.out.println(calculateMicroAveragedFMeasure(correctAnnotations,
				foundAnnotations.get(0)));
		for (int i = 1; i <= maxRank; i++)
			System.out.println(calculateRecallCeiling(correctAnnotations,
					foundAnnotations, i));
	...
	public static void main(String[] args) throws ConfigurationException,
		...
		SynonymMatrix matrix = FullRankSynonymMatrix.load(new File(
				matrixFilename));
		dnorm.runSynonymSimilarity(evalConfigurationFile, outputFilename,
				matrix);

		// FIXME Lexicon holds the Dictionary, but matrix holds one too; how do
