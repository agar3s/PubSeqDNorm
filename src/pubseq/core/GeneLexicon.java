...
/**
 * Gene implementation of {@link Lexicon}. This class also seems to work for
 * general data. Therefore, in later implementation, class {@link Lexicon}
 * should instead be modified directly/used as-is if no modification is needed.
 * In case of {@link Lexicon} we just use the native class as it only basically
 * contains ScoreDoc
 * 
 * @author raharjaliu
 *
...
public class GeneLexicon {
	...
	// TODO Refactor - this code now supports two different purposes: Lucene
	// searching & exhaustive searching

	private Normalizer norm;
	private Dictionary dict;
	private GeneNameAnalyzer analyzer;
	...
	public Map<Vector, Set<String>> vectorsToConceptIds; // TODO Decide how to
															// share this
	public Map<String, Set<Vector>> conceptIdToVectors; // TODO Decide how to
														// share this
	public Set<String> conceptIds; // TODO Decide how to share this

	public GeneLexicon(GeneNameAnalyzer analyzer) {
		norm = new Normalizer();
		dict = new Dictionary();
	...
	public void addConcept(String conceptId, String name, boolean isPreferred) {
		if (writer == null)
			throw new IllegalStateException(
					"Cannot add concepts after calling prepare()");
		try {
			conceptIds.add(conceptId);
			AnalyzedName analyzedName = new AnalyzedName(
					analyzer.getTokens(name), isPreferred);
			Set<String> conceptIds = analyzedNamesToConceptIds
					.get(analyzedName);
			if (conceptIds == null) {
				conceptIds = new HashSet<String>();
			...
			doc.add(new Field("conceptId", conceptId, Field.Store.YES,
	...
	public void addName(String name) {
		if (writer == null)
			throw new IllegalStateException(
					"Cannot add names after calling prepare()");
	...
	public void prepare() {
		try {
			if (writer != null) {
				...
				int count = 0;
				for (AnalyzedName analyzedName : analyzedNamesToConceptIds
						.keySet()) {
	...
	public GeneNameAnalyzer getAnalyzer() {
		return analyzer;
	}
	...
	public DoubleMatrix2D convertNameToVector(List<String> tokens) {
		final DoubleMatrix2D vector = DoubleFactory2D.sparse.make(dict.size(),
				1);
		for (String token : tokens) {
			int index = dict.getIndex(token);
	...
	public DoubleMatrix2D convertNameToVector(String name) {
		try {
			final DoubleMatrix2D vector = DoubleFactory2D.sparse.make(
					dict.size(), 1);
	...
	public boolean contains(String lookupText, GeneNameAnalyzer analyzer2) {
		...
		if (retval)
			System.out.println("lexicon returned \"" + name + "\" (" + docTerms
					+ ") as best match for \"" + lookupText + "\" ("
					+ queryTerms + ")");
		return retval;
	}
	...
	public Lookup getFuzzyLookup(int maxMatches, float minimumSimilarity,
			int prefixLength, int maxExpansions) {
		if (searcher == null)
			throw new IllegalStateException("Have to call prepare() first");
		return new FuzzyLookup(maxMatches, minimumSimilarity, prefixLength,
				maxExpansions);
	}
	...
	private class ExactLookup implements Lookup {
		...
		public ScoreDoc[] lookup(String lookupText) {
			try {
				...
				TokenStream tokenStream = analyzer.tokenStream("name",
						new StringReader(lookupText));
				while (tokenStream.incrementToken()) {
					CharTermAttribute term = tokenStream
							.getAttribute(CharTermAttribute.class);
					query.add(new TermQuery(new Term("name", term.toString())),
							Occur.SHOULD);
				}
	...
	private class FuzzyLookup implements Lookup {
		...
		public FuzzyLookup(int maxMatches, float minimumSimilarity,
				int prefixLength, int maxExpansions) {
			this.maxMatches = maxMatches;
		...
		public ScoreDoc[] lookup(String lookupText) {
			try {
				...
				while (tokenStream.incrementToken()) {
					CharTermAttribute term = tokenStream
							.getAttribute(CharTermAttribute.class);
					query.add(new FuzzyQuery(new Term("name", term.toString()),
							minimumSimilarity, prefixLength, maxExpansions),
							Occur.SHOULD);
				}
				TopDocs topDocs = searcher.search(query, maxMatches);
				ScoreDoc[] hits = topDocs.scoreDocs;
