...
/**
 * 
 * Gene Names context-specific implementation of original DNorm class
 * {@link DiseaseNameAnalyzer}. Only small modifications are needed since this
 * system seems to work for all kind of data. Therefore, in later
 * implementation, class {@link DiseaseNameAnalyzer} should instead be modified
...
public class GeneNameAnalyzer extends Analyzer {
	...
	public static final Set<String> DEFAULT_STOPWORD_SET = new HashSet<String>(
	...
	public static GeneNameAnalyzer getGeneNameAnalyzer(boolean useStopWords,
			boolean numericSynonyms, boolean nearSynonyms, boolean stem) {
		try {
			...
			if (nearSynonyms) {
				synonyms.add(new CharsRef("dominant"), new CharsRef("genetic"),
						false);
				synonyms.add(new CharsRef("recessive"),
						new CharsRef("genetic"), false);
				synonyms.add(new CharsRef("inherited"),
						new CharsRef("genetic"), false);
				synonyms.add(new CharsRef("hereditary"),
						new CharsRef("genetic"), false);
				synonyms.add(new CharsRef("disorder"), new CharsRef("disease"),
						false);
				synonyms.add(new CharsRef("abnormality"), new CharsRef(
						"disease"), false);
				synonyms.add(new CharsRef("absence"),
						new CharsRef("deficiency"), false);
				synonyms.add(new CharsRef("handicap"), new CharsRef(
						"deficiency"), false);
			}
			SynonymMap map = null;
			...
			return new GeneNameAnalyzer(stem, stopWords, map);
		} catch (IOException e) {
	...
	public GeneNameAnalyzer(boolean useStemming) {
		this(useStemming, null, null, true, 1, DEFAULT_CASE_FOLDING_LENGTH);
	}

	public GeneNameAnalyzer(boolean useStemming, Set<String> stopWords,
			SynonymMap synonyms) {
		this(useStemming, stopWords, synonyms, true, 1,
				DEFAULT_CASE_FOLDING_LENGTH);
	}

	public GeneNameAnalyzer(boolean useStemming, Set<String> stopWords,
			SynonymMap synonyms, boolean retainDigits, int minLength) {
		this(useStemming, stopWords, synonyms, retainDigits, minLength,
				DEFAULT_CASE_FOLDING_LENGTH);
	}

	public GeneNameAnalyzer(boolean useStemming, Set<String> stopWords,
			SynonymMap synonyms, boolean retainDigits, int minLength,
			int caseFoldingLength) {
	...
	public TokenStream tokenStream(String fieldName, Reader reader) {
		...
		if (caseFoldingLength >= 0)
			stream = new AcronymPreservingLowerCaseFilter(Version.LUCENE_36,
					stream, caseFoldingLength);
		if (stopWords != null)
	...
	public TokenStream reusableTokenStream(String fieldName, Reader reader)
		...
		if (streams == null) {
			...
			if (caseFoldingLength >= 0)
				streams.result = new AcronymPreservingLowerCaseFilter(
						Version.LUCENE_36, streams.result, caseFoldingLength);
			if (stopWords != null)
				streams.result = new StopFilter(Version.LUCENE_36,
						streams.result, stopWords);
			if (synonyms != null)
				streams.result = new SynonymFilter(streams.result, synonyms,
						false);
			// streams.result = new
			// AmericanBritishSpellingNormalizer(streams.result);
			if (useStemming)
	...
	public List<String> getTokens(String name) {
		try {
			...
			TokenStream tokenStream = tokenStream("name",
