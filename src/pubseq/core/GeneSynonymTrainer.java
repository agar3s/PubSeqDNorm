...
/**
 * Gene Implementation of {@link SynonymTrainer}. Only small modifications are
 * needed since this system seems to work for all kind of data. Therefore, in
 * later implementation, class {@link SynonymTrainer} should instead be modified
 * directly/used as-is if no modification is needed. It is important, to make
 * sure this class works as intended, that this class only calls
 * gene-implementation instead of the native DNorm methods.
 * 
 * @author raharjaliu
 *
 */
public class GeneSynonymTrainer {

	private SparseEquals s;
	private int maxRank;
	private GeneLexicon lex;
	private SynonymMatrix matrix;

	public GeneSynonymTrainer(GeneLexicon lex, SynonymMatrix matrix, int maxRank) {
	...
	public void trainArgmax(List<TrainingPoint> training, double lambda,
			double margin, List<TrainingPoint> holdout, int maxIterations) {
		...
		if (holdout != null) {
			...
			System.out.println("Iteration: " + iterations
					+ ", holdout performance=" + holdoutPerformance);
		}
		long start = System.currentTimeMillis();
		while (updates > 0 && holdoutPerformance <= previousHoldoutPerformance
			...
			System.out.println("Iteration: " + iterations + ", updates="
					+ updates + ", elapsed="
		...
		visualizeLookup("autosomal recessive disease", 10);
		visualizeLookup("neurological dysfunction", 10);
		System.out.println("Training complete. Iterations: " + iterations
				+ ", elapsed=" + (System.currentTimeMillis() - start));
	}

	public void trainArgmax(List<TrainingPoint> training, LearningRate l,
			double margin, List<TrainingPoint> holdout, int maxIterations) {
		int updates = Integer.MAX_VALUE;
		int iterations = 0;
		double previousHoldoutPerformance = Double.MAX_VALUE;
		...
		while (updates > 0 && holdoutPerformance <= previousHoldoutPerformance
			...
			System.out.println("Learning rate = " + lambda + ", iterations = "
					+ iterations);
			for (int i = 0; i < training.size(); i++) {
				System.out.println("Mention\t" + i + "\t"
						+ training.get(i).getName() + "\t"
						+ lex.visualizeVector(training.get(i).getVector()));
				updates += trainConcepts(training.get(i), lambda, margin);
				if ((i % 10) == 0)
					System.out.println(i + "/" + training.size() + ", updates="
							+ updates);
			}
			System.out.println("Iteration: " + iterations + ", updates="
					+ updates + ", elapsed="
	...
	public void trainArgmaxNEW(List<TrainingPoint> training, LearningRate l,
			int maxIterations, List<TrainingPoint> holdout) {
		int updates = Integer.MAX_VALUE;
		int iterations = 0;
		double holdoutPerformance = 0.0;
		if (holdout != null) {
		...
		List<Vector> vectors = new ArrayList<Vector>(
				lex.vectorsToConceptIds.keySet());
		...
		while (updates > 0 && iterations < maxIterations) {
			...
			System.out.println("Learning rate = " + lambda + ", iterations = "
					+ iterations);
			double alpha = 100.0 / (100.0 + iterations); // TODO Make
															// configurable
			...
			for (int i = 0; i < training.size(); i++) {
				...
				if (nameP != null) {
					Set<String> conceptIds = new HashSet<String>(
							lex.vectorsToConceptIds.get(nameP));
					DoubleMatrix2D dp = nameP.getVector();
					double dpScore = matrix.score(q, dp);
					System.out.println("Best name for \""
							+ training.get(i).getName() + "\", "
							+ training.get(i).getConceptId() + " is ["
							+ lex.visualizeVector(dp).replaceAll("\t", " ")
							+ "] @ " + dpScore);
					Collections.shuffle(vectors);
					// Find a vector with a higher score than the self-score &&
					// no concept overlap
					for (int j = 0; j < vectors.size(); j++) {
						...
						if (train)
							train = !containsAny(conceptIds,
									lex.vectorsToConceptIds.get(nameN));
						if (train) {
							System.out.println("q : " + lex.visualizeVector(q));
							System.out.println("dp: " + lex.visualizeVector(dp)
									+ "=" + dpScore);
							System.out.println("dn: " + lex.visualizeVector(dn)
									+ "=" + dnScore);
							System.out.println();
	...
	public void trainDictionary(LearningRate l, int maxIterations,
			List<TrainingPoint> holdout) {
		...
		while (updates > 0 && iterations < maxIterations) {
			...
			for (int i = 0; i < vectors.size(); i++) {
				...
				// Find a vector with a higher score than the self-score && no
				// concept overlap
	...
	public void trainSampled(List<TrainingPoint> training, double lambda,
			List<TrainingPoint> holdout, int maxRounds, int iterationsPerRound) {
		int updates = Integer.MAX_VALUE;
		int rounds = 0;
		double previousHoldoutPerformance = Double.MAX_VALUE;
		double holdoutPerformance = 0.0;
		if (holdout != null) {
			holdoutPerformance = getHoldoutPerformance(holdout);
			System.out.println("Round: " + rounds + ", holdout performance="
					+ holdoutPerformance);
		}
		...
		while (updates > 0 && holdoutPerformance <= previousHoldoutPerformance
			...
			for (int i = 0; i < iterationsPerRound; i++) {
				...
				TrainingPoint point = training
						.get(rnd.nextInt(training.size()));
				// Pick randomly from names for training point concept
				List<Vector> names = conceptIdToVectors.get(point
						.getConceptId());
				Vector nameP = null;
				if (names == null) {
					System.out.println("Unknown conceptID:"
							+ point.getConceptId());
				} else {
					nameP = names.get(rnd.nextInt(names.size()));
				}
				// Pick a concept randomly
				String conceptN = conceptIds
						.get(rnd.nextInt(conceptIds.size()));
				...
				if (nameP != null && nameN != null) {
					...
					check = check
							&& matrix.score(q, dp) - matrix.score(q, dn) < 1.0;
					if (check) {
						matrix.trainingUpdate(q, dp, dn, lambda);
						System.out.println("q : " + lex.visualizeVector(q));
						System.out.println("dp: " + lex.visualizeVector(dp)
			...
			System.out.println("Round: " + rounds + ", updates=" + updates
					+ ", elapsed=" + (System.currentTimeMillis() - start));
			rounds++;
			if (holdout != null) {
				holdoutPerformance = getHoldoutPerformance(holdout);
	...
	public void trainLexicon(double lambda, List<TrainingPoint> holdout,
			int maxIterations) {
		int updates = Integer.MAX_VALUE;
		int iterations = 0;
		...
		while (updates > 0 && iterations < maxIterations) {
			...
			for (Vector v : vectors) {
				Set<String> conceptIds = new HashSet<String>(
						lex.vectorsToConceptIds.get(v));
				RankedList<LookupResult> results = lookup(v.getVector());

				// Get the minimum of the maximum score for any correct concept
				// ID
				double minScore = Double.MAX_VALUE;
				Set<String> conceptIdsCopy = new HashSet<String>(conceptIds);
				for (int j = 0; j < results.size(); j++) {
					String resultConceptId = results.getObject(j)
							.getConceptId();
				...
				for (int j = 0; j < results.size() && resultN == null; j++) {
					String resultConceptId = results.getObject(j)
							.getConceptId();
					if (!conceptIds.contains(resultConceptId)
							&& results.getValue(j) > minScore) {
	...
	public void trainLexicon2(double lambda, double margin,
			List<TrainingPoint> holdout, int maxIterations) {
		Random r = new Random();
		int updates = Integer.MAX_VALUE;
		int iterations = 0;
		...
		while (updates > 0 && iterations < maxIterations) {
			...
			int progress = 0;
			for (Vector v : vectors) {
				double threshold = matrix.score(v.getVector(), v.getVector())
						- margin;
				Set<String> conceptIds = new HashSet<String>(
						lex.vectorsToConceptIds.get(v));
				...
				for (int i = 0; i < vectors.size() && cont; i++) {
					...
					train = train
							&& !containsAny(conceptIds,
									lex.vectorsToConceptIds.get(v2));
					if (train) {
						matrix.trainingUpdate(v.getVector(), v.getVector(),
								v2.getVector(), lambda);
						System.out.println("q : "
								+ lex.visualizeVector(v.getVector()));
						System.out.println("dp: "
								+ lex.visualizeVector(v.getVector()) + "="
								+ matrix.score(v.getVector(), v.getVector()));
						System.out.println("dn: "
								+ lex.visualizeVector(v2.getVector()) + "="
								+ matrix.score(v.getVector(), v2.getVector()));
						System.out.println();
						updates++;
					}
				}
	...
	private static boolean containsAny(Set<? extends Object> set1,
	...
	public void visualizeLookupOLD(String name, int max) {
		...
		for (int i = 0; i < Math.min(lookup.size(), max); i++) {
			...
			System.out.printf("\t%f\t%s\t%s", lookup.getValue(i),
					result.getConceptId(),
					lex.visualizeVector(result.getVector()));
			System.out.println();
		}
		System.out.println();
	}
	...
	public void visualizeLookup(String name, int max) {
		...
		for (int i = 0; i < Math.min(lookup.size(), max); i++) {
			...
			System.out.printf("\t%f\t%s\t%s", lookup.getValue(i),
					result.getConceptId(), lex.visualizeVector(resultVector));
			System.out.println();
	...
	public void visualizeScore(DoubleMatrix2D nameVector,
			DoubleMatrix2D resultVector) {
			...
			if (nameVector.get(i, 0) != 0.0) {
				...
				for (int j = 0; j < size; j++) {
					if (resultVector.get(j, 0) != 0.0) {
						...
						System.out.printf("\t\t%s\t->\t%s\t%f", nameToken,
								resultToken, m.w.get(i, j));
						System.out.println();
					}
				}
			}
	...
	private int trainConcepts(TrainingPoint point, double lambda, double margin) {
		...
		if (nameP != null) {
			for (String conceptId : lex.conceptIds) {
				...
				check = check && !s.equals(q, dn);
				check = check
						&& matrix.score(q, dp) - matrix.score(q, dn) < margin;
				if (check) {
					matrix.trainingUpdate(q, dp, dn, lambda);
					// System.out.println("q : " + lex.visualizeVector(q));
					// System.out.println("dp: " + lex.visualizeVector(dp) + "="
	...
	private int getSynonymRank(TrainingPoint point) {
		...
		for (int i = 0; i < results.size(); i++) {
			if (results.getObject(i).getConceptId()
					.equals(point.getConceptId()))
				return i;
		}
		return maxRank;
	}
