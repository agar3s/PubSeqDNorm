...
import dnorm.TrainSynonymMatrix;
 ...
 * @author raharjaliu
 * 
 *         PubSeq implementation of {@link TrainSynonymMatrix}
 */
public class TrainGeneSynonymMatrix {
	...
	public static void main(String[] args) throws ConfigurationException {
		...
		DiseaseNameAnalyzer analyzer = DiseaseNameAnalyzer
				.getDiseaseNameAnalyzer(true, true, false, true);
		EvalDNorm dnorm = new EvalDNorm(analyzer, medicFilename,
		...
		syn.trainArgmax(training, new SynonymTrainer.FixedLearningRate(lambda),
				1.0, holdout, maxIterations);
		matrix.write(new File(matrixFilename));
	}

	private static List<TrainingPoint> loadDataset(EvalDNorm dnorm,
			String configurationFile, SynonymTrainer syn)
